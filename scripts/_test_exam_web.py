...
def test_http_deny_and_data(tmp_bank: str):
    ...
    try:
        ...
        check("确认提交答卷？提交后封存，只能看批改。" in html, "exam submit confirm copy")
        check("window.confirm(I18N.submitConfirm)" in html, "exam submit asks before post")
        check("网络错误，识别未完成。" in html, "exam ocr network error is plain")
        check('setOcrMsg("网络错误："' not in html, "exam ocr hides raw exception text")
        check('id="ocrFab"' in html, "exam OCR floating button")
